...
public class Nuvem {
	...
	public static void newUser(String _user, String _password) {
		User user = null;
		user = new User();
		user.setName(_user.trim()); // trim to remove spaces
	...
	public static boolean createNetwork() {
		...
		try {
			for (int i = 0; i < 5; i++) {
				...
				System.out
						.println("Criado No:" + (i) + " Porta:" + nos.get(i).getPorta() + " Id: " + nos.get(i).getID());
				Nuvem.port++;
				Nuvem.nNos++;
			}
			return true;
	...
	/**
	 * @param _name
	 * @return Create a hash key from a name of user
	 */
	 ...
	 * @param _password
	 * @param no
	 * @return Method called by company that need validate a user in your system
	 */
	public static String validateUser(String _user, String _password) {
		String codePass = null;
		String aux = null;
		String aux2 = null;
		User user = new User();
		StringKey myKey = new StringKey(_user.trim());
